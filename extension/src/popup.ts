...
async function run(q: string): Promise<void> {
  ...
  try {
    ...
    if (mine === generation) render(quick.hits, `${quick.took_ms} ms · lexical`);
  } catch (e) {
  ...
  try {
    ...
    if (mine === generation) {
      ...
      render(full.hits, `${full.took_ms} ms · ${labels || "ranked"}`);
    }
  } catch (e) {
...
function render(hits: Hit[], note: string): void {
  ...
  for (const h of hits) {
    ...
    meta.className = "meta";
    meta.textContent = [h.domain, h.folder, h.via].filter(Boolean).join(" · ");
    a.appendChild(meta);
