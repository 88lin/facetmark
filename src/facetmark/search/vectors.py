...
def content_list(
    ...
    if not vec_tables_exist(conn):
        return []
    return [bid for bid, _ in knn_content(conn, query_vec, limit)]
...
def intent_list(
    ...
    *,
    ...
    out: list[int] = []
    seen: set[int] = set()
    for intent_id, _dist in hits:          # already ordered by distance
...
def vector_lists_from_vec(
    ...
    *,
    ...
    if want_content:
        c = content_list(conn, query_vec, limit=limit)
        if c:
            out["content"] = c
    if want_intent:
        i = intent_list(conn, query_vec, limit=limit)
        if i:
            out["intent"] = i
    return out
