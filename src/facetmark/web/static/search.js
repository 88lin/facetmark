...
/**
 * The mixture behind one score.
 *
 * `contributions` is `w / (k + rank)` per facet, straight from the fuser, so
 * the numbers are already proportional to what each path contributed -- they
 * are normalised here only so the bar fills its track. Drawn only when at
 * least two paths found the page: a full-width single-colour bar under every
 * one-path result is a row of decoration that says nothing.
 */
function contributionBar(h) {
  const c = h.contributions;
  if (!c) return null;
  const bands = FACET_ORDER.filter((f) => c[f] > 0).map((f) => ({ f, v: c[f] }));
  if (bands.length < 2) return null;
  const total = bands.reduce((a, b) => a + b.v, 0);
  if (!(total > 0)) return null;
  const label = bands
    .map((b) => `${t(FACET_KEYS[b.f])} ${Math.round((b.v / total) * 100)}%`)
    .join(", ");
  return fbar(
    bands.map((b) => ({ tone: FACET_TONE[b.f], frac: b.v / total })),
    t("hit.mix", { parts: label }),
  );
}
...
function hitCard(h, rank, neighbour) {
  ...
  if (neighbour) {
    ...
    // One badge per path that found this page, always in the same order and
    // always in that path's colour. The four-way recall is the product's whole
    // argument and it used to be invisible on the one screen that proves it.
    for (const f of FACET_ORDER) {
      if (!(h.facets ?? []).includes(f)) continue;
      marks.appendChild(fbadge(FACET_TONE[f], t(FACET_KEYS[f]), tOr(`${FACET_KEYS[f]}.why`, "")));
    }
    for (const f of h.facets ?? []) {
      // A facet the server grew and this page has not learned about yet.
      if (!FACET_KEYS[f]) marks.appendChild(pill("vec", f));
    }
  }
  ...
  const bar = contributionBar(h);
  if (bar) a.appendChild(bar);
 ...
 * filters, the date window, the sort. Shown in the status line so a user (or
 * an agent) can see that "domain:github.com -pinterest" did what they meant
 * rather than being searched as literal words. `null` (plain query) renders
 * as nothing at all -- the absence of the row is the information that no
 * grammar was involved.
 */
function filterSummary(filters) {
  if (!filters) return "";
  const parts = [];
  for (const f of filters.field_filters ?? []) {
    const joiner = f.values.length > 1 ? "|" : "";
    parts.push(`${f.negated ? "-" : ""}${f.field}:${joiner}${f.values.join("|")}`);
  }
  if (filters.dates) {
    const [from, to] = filters.dates;
    if (from != null && to != null) parts.push(`added:${from}..${to}`);
    else if (from != null) parts.push(`after:${from}`);
    else if (to != null) parts.push(`before:${to}`);
  }
...
function emptyPanel(queried) {
  ...
  const into = ui.panel;
  if (s && !s.bookmarks) {
    // A command to type is not an exit for someone who opened a web page.
    // The button is the exit; the command stays for the reader who would
    // rather use a terminal, which on this project is most of them.
    return showPanel({
      title: t("empty.import.title"),
      body: t("empty.import.body"),
      cmd: t("empty.import.cmd"),
      extra: [btn(t("empty.setup"), "primary", () => S.go("setup"))],
      into,
    });
  }
  ...
  if (s && s.bookmarks && !s.with_body && !vec) {
    return showPanel({
      ...
      body: t("empty.index.body", { n: count(s.bookmarks, S.lang) }),
      cmd: t("empty.index.cmd"),
      extra: [btn(t("empty.setup"), "primary", () => S.go("setup"))],
      into,
    });
  }
