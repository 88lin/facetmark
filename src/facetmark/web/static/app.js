...
/**
 * Every view, and the five of them that are tabs.
 *
 ...
 * numbers.
 *
 * Setup and Settings are routable but not tabs. Setup is a page you are meant
 * to visit once, and a permanent tab for a finished job is a permanent
 * reminder of it; Settings is one gear in the header, where every other
 * program on the machine also keeps it. Six tabs to reach two screens nobody
 * opens daily would cost the five that are opened daily their room.
 */
const VIEWS = {
  ...
  setup: { panel: "#view-setup", mod: setup, tab: false },
  settings: { panel: "#view-settings", mod: settings, tab: false },
};
/** Every name the hash may carry. Five of them are also tabs. */
...
function viewName() {
  ...
  return ROUTES.includes(name) ? name : "search";
}
...
function route() {
  ...
  // The gear is the only way in to Settings, so it carries the current-page
  // mark the tab strip would otherwise carry for it.
  if (name === "settings") ui.gear.setAttribute("aria-current", "page");
  else ui.gear.removeAttribute("aria-current");
...
function wire() {
  ...
  // The gear toggles rather than only opening: a settings panel you reached by
  // gear should close the same way, and the tab strip cannot show it as
  // current because it has no tab.
  ui.gear.addEventListener("click", () => S.go(viewName() === "settings" ? "search" : "settings"));
...
async function boot() {
  ...
  S.go = (name) => {
    const next = `#/${ROUTES.includes(name) ? name : "search"}`;
...
/**
 * Send a reader with no library to the page that explains how to get one.
 *
 * Only from the default landing spot. Someone who typed `#/library` asked for
 * the library, and answering a direct request with a different page is the
 * behaviour that makes tools feel like they are arguing with you. An empty
 * library is the one condition checked here: whether the model is configured
 * is a question for `/admin/settings`, and Setup asks it itself once it is on
 * screen rather than making every cold start pay for the round trip.
 */
function firstRun(stats) {
  const asked = location.hash.replace(/^#\//, "");
  if (asked && asked !== "search") return;
  if (stats && (stats.bookmarks === 0 || !stats.vectors?.[0])) S.go("setup");
}
