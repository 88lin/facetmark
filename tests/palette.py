...
def gradient_stops(value: str) -> list[str]:
    """The colour stops of a gradient value, in declaration order.

    Angles (``145deg``), side keywords (``to bottom``) and bare positions
    (``58%``) carry no colour and drop out. ``transparent`` drops out too: it
    paints nothing, so it cannot be the surface anything is read against.
    """
    value = value.strip()
    if not GRADIENT.match(value):
        return []
    inner = value[value.index("(") + 1 : value.rindex(")")]
    out = []
    for part in _split_top_level(inner):
        colour = first_colour(part.strip())
        if colour:
            out.append(colour)
    return out
...
def first_colour(value: str) -> str | None:
    ...
    A gradient resolves to its *first* colour stop. That is only the right
    answer because the stylesheet writes the strongest stop first, which is
    not a convention the reader should have to take on trust --
    ``test_every_gradient_leads_with_its_strongest_stop`` measures it.
    """
    value = value.strip()
    if GRADIENT.match(value):
        stops = gradient_stops(value)
