...
### 修复（把查询语言真的跑起来之后发现的四个）

四个都不是测试挂了，而是把语法一条条敲进去之后才看见的——所以每条都补了会失败的回归测试。

- **`sort:date` 单独用时，两个入口给出两个答案。** 首屏（`quick_search`）把任何
  「没有自由文本」的查询当浏览，返回整库按时间倒序；完整管线（`search`）要求
  「至少有一个过滤器」，于是同一条查询在几百毫秒后被替换成空列表——用户看到结果
  闪一下就没了。现在两边读同一个属性 `ParsedQuery.is_browse`：没有可排序的文本、
  但查询确实要求了什么（过滤器、否定，或仅仅一个顺序），就是一次浏览。空查询不是
  浏览——什么都没输入不等于要求全部。
- **解析不了的过滤值被静默丢弃。** `docs/query-language.md` 一直承诺
  `added:90d` 这类值会出现在响应的 `filters.ignored` 里；实际上它由
  `filter_sets` 第三个返回值报告，而所有调用方都把它丢掉了，于是响应把
  `added:90d` 回显成一个「已生效的过滤器」，同时在检索里根本没用它。校验挪进解析器
  （和 `sort:weird` 的检查并排——它需要的信息里没有数据库），并且
  `filters` 回显的条件从 `has_syntax` 改成「`echo()` 非空」：一条语法全部解析失败的
  查询此前回显 `null`，看起来像纯文本查询。
- **MCP 工具描述宣传了四个不存在的语法。** `host:`、`after:30d`、`kuber*` 前缀、
  `sort:open_count` 里只有前面三个之一能用。前三个中有三个值得实现——`host:`
  （主机名子串，`domain:` 是站点）、`before:`/`after:` 读裸时长为年龄
  （`after:30d` = 最近 30 天，与 `added:<30d` 同义，折叠发生在校验之前）、
  `sort:opened`（打开次数最多在前，别名 `sort:open_count`）——都是过滤/排序，
  不动默认排名。`kuber*` 这种自由词前缀匹配会改变默认检索，属于要先注册协议的变更，
  因此从描述里删掉而不是实现。
- **karakeep 推过来的 tags 查不到。** 桥接把 tags 映射成 `folder` 字符串（喂
  folder-peer 上下文特征，这是刻意的近似）与 `enrichment.topics`，但没写进
  `bookmark.tags`，所以 `tag:work` 找不到 karakeep 的页面——而 karakeep 的 tags
  正是这个过滤器要的词汇表。现在同时写进 `bookmark.tags`，重复推送取并集而不是覆盖：
  这一行可能是被收养的浏览器导入，本来带着用户自己的标签。保留了「模型写过的
  enrichment 不被 karakeep 的词覆盖」这条已实测的规则——那一行的词法索引里仍然
  没有 karakeep 的 tags。
- **`site:` 的联想补全什么都不给。** 值补全的分支比较的是用户敲的名字而不是规范
...
里说它是「最快看清输出形状、顺便验证装好了」的办法，也就是新用户看到的第一屏，而我整场
都没跑过它。

- **`demo` 每行结果都带一个字面的 `None`。** `via` 只在图扩展那一组上有值，而 demo 打印
  的是 `resp.hits` 不是 `resp.expanded`，所以它永远是 `None`：
  `1. Why chromadb changes the recall story  None 0.0776`。
- **`demo` 把分阶段计时字典当数字格式化了**，于是一个 16 位精度的 Python dict repr 糊在
  句子中间、横跨三行：`target at rank 2 in {'understand': 1.0566999999355176, ...} ms`。
  **这是个已经记录过的 bug 类**：CHANGELOG 里写着扩展端犯过同一个错、打印出
  `[object Object] ms`，并且为此加了跨语言契约测试——而这条命令上还在。
- 顺带：不带 `--keep` 时它打印数据库路径，然后把文件删掉；现在会说清楚那是临时文件。
  「目标不在前五」原来渲染成一个孤零零的 `-`。
- 试过在每行显示命中的面，**又改掉了**：demo 跑的是四条路全开的档，所以每行都是
  `content + intent + lex_seg + lex_tri`，一条恒定不变的信息把行撑到换行。改成每行显示
  域名（和真实结果行一样），每条查询显示各面的候选数——后者才随查询变化。
- **`facetmark stats` 把嵌套字典塞进表格单元格。** `cold_layer` 整块以 JSON 形式落在一格
  里、横跨三行，里面还有个裸的 unix 时间戳 `cutoff_ts 1758886375`；列表值则是 Python
  list repr。现在嵌套组变成缩进的行、`_ts` 结尾的值显示成日期、列表用逗号连接、空字典
  说 `none`。`--json` 的结构完全不动——表格是给人读的，JSON 才是契约。
- **`fm` 这个短名一直存在却没写进文档。** pyproject 里和 `facetmark` 一起注册，
  `facetmark health` 的提示里用的就是它，而两份 README 从没提过。补进命令表，并且把上一轮
  那条「README 里提到的命令必须真的存在」的守卫扩展到两个二进制名。
...
也能启动、可它什么都搜不出来——到底哪一步错了。** 移植的是它的形态：一串
`{name, status, message}`，一行一条，`--json` 可机读，任何一条是 error 就非零退出。

两条规则也是它的，而且正是这条命令值得被信任的原因：

- **它不修任何东西。** 一条会修改被诊断对象的诊断命令，跑第二遍含义就变了，而且一个
  还没想好要不要改的人根本不敢跑它。这里每一项都只读。写这条时发现自己踩了一次：
  最初用 `_open()` 打开库，而 `open_db` **会自动迁移**——也就是在报告 schema 之前先把
  schema 改掉了。改成 `connect()`，顺带 schema 那一项才真的看得见落后的库。
- **它不调用模型。** 要报告「你的端点连不上」就得发一次请求，而一条会花钱、或者把查询
  泄漏给第三方的诊断命令，人们会学会不去跑它。不用联网能查的都查（配了哪个后端、有没有
  key、已存向量和设置对不对得上），其余的只作为配置报告，不冒充健康状况。

检查项对着这个项目真实的失败模式挑的，多数是因为代码里本来就有防御：

| 它会告诉你 | 否则你只会看到 |
|---|---|
| 已存向量是 64 维而设置写 1536 | 索引跑到一小时后抛 `SchemaMismatch` |
| 向量表存在但是空的（chat-only 的 key 长这样） | 每条查询都返回空页 |
| 库里有书签但词法索引是空的 | 搜什么都搜不到 |
| schema 落后 / 来自更新的版本 | 打开时才报错 |
| 哪些设置来自环境变量、因而文件和界面都改不动 | 「我改了配置文件怎么没生效」 |

每一条发现都写明该跑哪条命令。`tests/test_diagnose.py` 里除了逐项断言，还钉住了那条
承诺本身：跑两遍 `run_checks`，数据库对象、行数、meta 逐一比对必须完全一致。

### 性能（`tag:` 过滤与补全）

在一个更接近真实的标签分布上重测（24 个标签、长尾、65% 的书签带标签）发现：同样的
选择性下 `tag:` 比 `domain:` 贵 2.6 倍——`tag:tag23` 只命中 1.4% 却要 35 ms，而
`domain:github.com` 命中 0.5% 只要 13 ms。贵在 `json_each(b.tags)` 必须把每一行的
JSON 都解析一遍，跟筛掉多少行无关。

先在 JSON 原文上做一次廉价预筛，让解析只落在可能命中的行上：

| | 之前 | 之后 |
|---|---|---|
| `postgres tag:tag12`（2.6% 命中） | 36 ms | **17 ms** |
| `tag:tag12` 浏览 | 32 ms | **11 ms** |
| `tag:tag1` 补全 | 44 ms | **9.4 ms** |

预筛是一个**超集**：标签由 `json.dumps` 写入，所以元素 `work` 在原文里必然以
`"work"` 出现，LIKE 命中的一定包含真正命中的，背后的 `EXISTS` 仍然是唯一裁判。超集
不可能改变答案——这是允许它写得这么糙的唯一理由（标签里带 `%` 也只会让预筛更宽）。

它依赖一件事：标签以 `ensure_ascii=False` 序列化。否则 `工具` 会被存成
`工具`，原文里找不到字面元素，**中文标签会被静默筛掉**——在这个项目最主要
的用户群上。五个写入点现在都是 `ensure_ascii=False`，并且
`test_a_cjk_tag_survives_the_prune` 把这条约定钉住了：谁把某个写入点改回默认，这条
测试就红。

裸 `tag:`（没有片段可筛）不加预筛：每一行带标签的都得访问，加了实测**更慢**
（71 → 92 ms）。

**没有做的：给标签建一张联表。** 那是这个问题的根治办法，但维护它需要触发器，而
`json_each(NEW.tags)` 在触发器里遇到任何非 JSON 的值时行为不可预测——我试过的四种
守卫写法（`WHEN json_valid(...)`、`CASE`、`iif`）都挡不住，而且失败与否取决于之前
插过哪些行。一条会让「插入另一本书签」随机失败的触发器，不值得用来换 30 ms。
...
- **`facetmark init` 是快速开始的第一行，而它不存在。** 一个新读者照 README 敲下的
  第一条命令，得到的是 `No such command 'init'`。同一张命令表里还有 `import-json`、
  `fetch`/`enrich`/`embed`/`intents`/`edges`、`selfcheck-embed`、`export`——九个
  没有一个存在。实际是 19 条（`--help` 数得出来），`config` 是一个带 `path`/`show`
  的子命令组。两份 README 的命令表按真实的 19 条重写；没有单独的建库步骤，任何打开
  库的命令都会顺手把它建出来；`import` 一条命令同时吃 Netscape HTML 与 Chrome JSON，
  按文件形状识别，所以 `import-json` 这一行本来就该是它。排错表里那两条
  `facetmark enrich --force` / `embed --force` 改成 `index --force`。
  新增 `tests/test_cli.py::TestTheDocumentedCommands`：把 README 里当作命令给出的
  每一个 `facetmark X`（围栏代码块里的行、反引号里的 span）对着 typer 注册表核一遍。
  散文会漂移，命令表是一串承诺。
- **`POST /open` 用一个已经不存在的 id 调用时返回 500，并把 SQL 错误原文放进响应体。**
  客户端完全可能拿着过期的 id——扩展画好列表、那一行被删掉、用户点了下去——`interaction`
  的插入撞上外键。现在 `record_open` 返回它有没有找到那条书签，路由回 404。

### 界面

- **时间线改成从左到右由旧到新。** 端点按新到旧返回（对列表是对的顺序），但对坐标轴
  是错的：一张从右往左走的时间柱状图，凡是见过另一张的人都会读反。柱子下面加了一条
  基线（没有它，七根高矮不一的柱子是浮着的，眼睛得从最矮那根去猜基线在哪），今天那
  一列用字重而不是颜色标出来——七根柱子里出现第二种颜色会被读成分类。
- **时间线的柱子此前没有焦点圈。** 一个可聚焦的按钮没有可见焦点指示，是 WCAG 2.4.7。
- **`prefers-reduced-motion` 此前只关掉了骨架屏动画和两个 transform。** 会长高的柱子、
  会滑动的进度条、会旋转的折叠箭头都是运动。颜色过渡保留：那不是移动，去掉之后 hover
  和按下状态会像重绘。
- **标签徽章不再借用内容面的色相。** 结果行上每一个带色的 pill 都是排序器的一个判断，
  而 `mute` 就是内容面的颜色；标签是读者自己的词，给它一个面的颜色等于宣称那个面命中了它。
  现在用无色调的基础 pill。

### 改进（过滤器现在参与检索，而不只是裁剪检索结果）

- **`postgres domain:github.com` 在候选深度之外一无所获。** 过滤器此前在融合*之后*
  裁切候选池：各面为「postgres」各取前 50 条，过滤器把这 50 条全部滤掉，于是一条库里
  确实存在、只是排在第 200 名的 github 页面根本没机会出现——返回空页。现在正向过滤
  给出的可选 id 集合被推进 FTS 查询的 `LIMIT` *之前*（一个 `json_each` 参数，不是
  长度不定的 `IN (?,?,…)`），不合格的行不再占用排名坑，合格的深名次行被顶上来。
  向量面没法给 `vec0` 的 KNN 加 `WHERE`，所以改成多取 5 倍再求交——够用来防止一个
  严格的过滤器把整页清空，而 KNN 的开销随请求量增长，所以不是 50 倍。
  否定式**刻意不下推**：`-facebook` 的可选集合是「整库减一页」，为了删掉一行去物化
  它是白做功。融合后的 `apply_filters` 照旧运行，所以*答案*两边一致——这次改的只是
  「允许各面看多深」。不带过滤器的查询完全不走这条路径，默认排名逐字节不变。
  一个 120 页的库里，那条只在第 120 名的合格页面，改之前返回 `[]`，改之后返回它。

### 前端与扩展

- **扩展弹窗现在回显服务端解析出的语法**，包括被忽略的 token（`⚠`）。之前一条
  打错的过滤器在弹窗里完全看不出来：结果列表安静地少用了一半你输入的东西。
- **扩展弹窗渲染 `#tag` 徽章**，并且 `api.ts` 的 `Hit`/`SearchResponse` 类型补上
  `tags`/`filters`/`sort`——那三个字段由 `tests/test_api.py` 的跨语言契约测试盯着，
  声明了就必须真的发。
- **只有 http(s) 才写进 `<a href>`。** 书签可以是 `javascript:`（bookmarklet 也是
  书签），而中键/Ctrl 点击不经过 click 处理器；扩展页面是特权上下文。
  `chrome.tabs.create` 本来也会拒绝这类 scheme，所以这一行同时修掉一个未捕获的 rejection。
- **网页首屏也回显语法。** 纯过滤查询是由首屏*回答*的，一个几百毫秒后才出现的
  「已忽略」提示没人还在读。
- **时间线上没有保存记录的那一天不再是按钮。** 柱子还在（七根柱子带缺口才是一周的
  形状），但它不再占一个 tab 停靠点、也不再跑一次保证为空的搜索。

### 文档与站点

- **站点补上查询语言**：`guide` 新增一节（字段表、否定/短语/多选/通配、日期、
  排序与浏览、标签），首页多一张「它说一门过滤语言」的卡片，`webui` 页写上时间线与
  语法补全，结果行说明里加上 `#tags`。中英同步。
- **站点的过时数字**：测试数 1,514 → 1,619，命令数 16 → 18（`crawl`、`update` 也
  补进了命令表），路由数 27 → 29。README 两语的徽章与正文同步到 1,619。
- **英文 README 的目录树此前整棵是中文注释**，已改成英文，并补上 `crawl.py`、
  `search/querylang.py`、`docs/landing/`。
- **站点的双语一致性检查此前只覆盖三页**（`quickstart`/`guide`/`measured`）。
  `webui`、`config`、`integrations` 三页既不检查结构漂移、也不检查「已提交的 HTML
...
- **书签终于保存导入器早已解析出的标签。** `RawBookmark.tags` 从 Netscape 的
  ...
  （HTTP `/bookmark` 与 MCP `save_bookmark`）接受 `tags` 参数，重复保存同样并集；
