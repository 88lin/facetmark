...
from facetmark.search.pipeline import CONFIGS, DEFAULT_FACET_WEIGHTS, EXPLORATORY
from facetmark.search.rrf import rrf
from facetmark.search.understand import classify

#: The media compared empirically. Both are lexical, so both run without a
#: model; they stand in for "one facet" and "more than one facet".
...
def fused_scores(conn: sqlite3.Connection, text: str, medium: str,
                 settings: Settings) -> list[tuple[int, float]]:
    """Fused (id, score) for one query in one lexical medium."""
    cfg = EXPLORATORY[medium]
    lists = lexical_lists(conn, text, limit=settings.candidates_per_facet)
...
def replay(db: str, queries: list[dict], settings: Settings) -> dict:
    ...
    try:
            ...
            for q in queries:
                ...
                for r in PROBE_RANKS:
                    ...
                    if landed == 1:
                        reached_first[r] += 1
            per_medium[medium] = {
                "facets": sorted(EXPLORATORY[medium].facets),
        ...
        for q in queries:
            scores = fused_scores(conn, q["text"], "lex_only", settings)
            if not scores:
                continue
            u = classify(q["text"])
    ...
    return {
        ...
        "observed_boosts": {
            ...
            "anchors": "lex_only fused hits (no model required)",
            "by_query_type": {
                g: {
                    "candidates": len(v),
...
def run(args) -> None:
    ...
    result = {
        ...
        "replay": replay(args.db, queries, settings),
        "caveat": (
